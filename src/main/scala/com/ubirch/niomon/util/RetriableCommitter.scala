...
// NOTE(mrobakowski): code here is based on from akka.kafka.scaladsl.Committer
object RetriableCommitter extends StrictLogging {
        ...
        def doCommit(retriesLeft: Int, currentBackoff: Int): Future[Done] = offsetBatch.commitScaladsl()
          .recoverWith {
            case ex: RetriableException if retriesLeft > 0 =>
              logger.warn(s"Retrying batch commit in $currentBackoff ms ($retriesLeft retries left); " +
  ...
  def sink(settings: CommitterSettings, retries: Int, backoffFactor: Double = 1.5, initialBackoffMs: Int = 200, maxBackoffMs: Int = 10000)
